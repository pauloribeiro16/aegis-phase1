...
class UnifiedInvoker:
    ...
    def __init__(
        ...
        *,
        ...
        # CORR-110: provider=vllm falls back to ChatOpenAICompat's
        ...
        if base_url:
            ...
            from aegis_phase1.llm.openai_compat import DEFAULT_BASE_URL
        ...
        if provider == "minimax":
            ...
            # CORR-110: OpenAI-compatible HTTP backend (vLLM / TGI /
            # SGLang). The chat adapter reads AEGIS_VLLM_BASE_URL /
            # AEGIS_VLLM_API_KEY / AEGIS_VLLM_TIMEOUT from env inside
            # its own model_validator, so we leave the env-resolvable
            # fields at the module default and only pass the values
            # that the UnifiedInvoker explicitly resolved (model,
            # base_url, max_tokens). Passing timeout= would shadow the
            # env override — same convention as ChatMinimax.
            from aegis_phase1.llm.openai_compat import ChatOpenAICompat

            self.chat = ChatOpenAICompat(
                model=self.model,
                api_key=self.api_key,
                base_url=self.base_url,
                max_tokens=self.max_tokens,
            ...
            logger.info(
                "UnifiedInvoker.__init__: provider=vllm, chat=%s model=%s base_url=%s",
                type(self.chat).__name__,
                self.chat.model,
                self.chat.base_url,
            )
    ...
    def _ensure_ollama(self, source: str) -> None:
        ...
        the API is down). CORR-110: same skip applies to the vLLM
        provider — OpenAI-compatible servers expose ``/v1/models`` but
        not ``/api/version``, and the LLM call itself fails fast if the
        server is down.
        """
        # CORR-062 S2 / CORR-110: MiniMax gateway and OpenAI-compatible
        # HTTP servers (vLLM / TGI / SGLang) have no ``/api/version``
        # equivalent — skip the probe. The first real call will surface
        # any network / auth / model errors as ConnectionError or
        # HTTPError, which invoke_raw / invoke_spec already handle.
        if self.provider in ("minimax", "vllm"):
            return
