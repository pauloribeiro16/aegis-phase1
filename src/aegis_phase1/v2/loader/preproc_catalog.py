...
Layout (verified 2026-08-05 against `preproc_out/` generated by
`scripts/preprocess/pipeline.py`):
    preproc_out/
      entities/{subdomains,srs,sos,csfs,clauses,pairs,articles,ambiguities}/...
      audit/{csf_mapping_report.json, so_sr_coherence_report.json}
      index/{entities.json, by_regulation.json, by_subdomain.json,
             cross_references.json}

Historical note: this loader previously hard-coded `3-entities/` and
`4-reference_and_meta/` as the roots — that layout was an intermediate
spike from an earlier pipeline version and was dropped in favour of the
flat `entities/`+`audit/`+`index/` structure emitted by the v2 pipeline.
The constants below were corrected in CORR-099-prep.
"""
...
class AuditReport(_TolerantModel):
    """Aggregated gate reports from preproc_out/audit/.
...
class EntitiesIndex(_TolerantModel):
    """Pre-computed lookup tables from preproc_out/index/.
...
class PreprocCatalogLoader:
    ...
    def __init__(self, preproc_root: Path | str = "preproc_out") -> None:
        ...
        # CORR-099-prep: paths updated to match the flat layout emitted
        # by scripts/preprocess/pipeline.py (`entities/`, `audit/`,
        # `index/` directly under preproc_root). The previous
        # `3-entities/` + `4-reference_and_meta/` constants came from an
        # intermediate spike that is no longer on disk.
        self.entities_root = self.preproc_root / "entities"
        ...
        self._audit_dir = self.preproc_root / "audit"
        self._index_dir = self.preproc_root / "index"
    ...
    def load_audit(self) -> AuditReport:
        ...
        Sources:
          - audit/csf_mapping_report.json
          - audit/so_sr_coherence_report.json
        """
    ...
    def load_index(self) -> EntitiesIndex:
        """Load pre-computed lookup tables from index/."""
