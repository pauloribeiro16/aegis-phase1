...
def _extract_category_v11_refs(
    ...
    Returns a dict keyed by v2.0 category id (e.g. ``DE.AE``), with values
    ``[(xlsx_row, category_text, [v11_ids...])]``. A category header is any
    row where column B (Category) is populated and column C (Subcategory)
    is empty. The informative references cell (column E) is scanned for
    v1.1 ID tokens of the form ``FUNC.CAT-N`` where FUNC ∈ {ID, PR, DE,
    RS, RC} and CAT is one of the 23 v1.1 category codes.

    Example: the DE.AE category header at xlsx row 169 cites
    ``CSF v1.1: DE.DP-2`` in its Informative References cell — this
    signals that the entire DE.AE category absorbed v1.1's DE.DP-2.
    """
    import openpyxl as _openpyxl

    # Known v1.1 category codes (NIST CSWP 41, 2018). Anything outside
    # this set is either a v2.0 ID (will be filtered separately) or a
    # spurious match from another framework's reference (e.g. "GV.CT-5"
    # from SP 800-221A, which is NOT a CSF v1.1 subcategory).
    _V11_CATS = {
        "ID.AM", "ID.BE", "ID.GV", "ID.RA", "ID.RM", "ID.SC",
        "PR.AC", "PR.AT", "PR.DS", "PR.IP", "PR.MA", "PR.PT",
        "DE.AE", "DE.CM", "DE.DP",
        "RS.AN", "RS.CO", "RS.IM", "RS.MI", "RS.RP",
        "RC.CO", "RC.IM", "RC.RP",
    }
    ...
    for row_idx, row in enumerate(ws.iter_rows(values_only=True), start=1):
        if row_idx < 3:
            continue
        col_a, col_b, col_c, _, col_e = (list(row) + [None] * 5)[:5]
        # Propagate function
        if col_a and isinstance(col_a, str) and col_a.strip():
            m_fn = re.match(r"^([A-Z]{2})", col_a.strip())
            if m_fn:
                current_function_id = m_fn.group(1)
        # Category header: col B populated, col C empty
        if not col_b or (isinstance(col_b, str) and not col_b.strip()):
            continue
        if col_c and isinstance(col_c, str) and col_c.strip():
            continue
        cat_text = str(col_b).strip()
        cat_id_m = re.search(r"\(([A-Z]{2}\.[A-Z]{2})\)", cat_text)
        if not cat_id_m:
            continue
        cat_id = cat_id_m.group(1)
        # Skip if category doesn't match the current function
        if current_function_id and not cat_id.startswith(current_function_id + "."):
            continue
        refs_text = str(col_e) if col_e else ""
        # Match v1.1 IDs strictly: FUNC.CAT-N where FUNC.CAT is in the
        # known v1.1 set and N is 1-2 digits without leading zero.
        v11_ids: list[str] = []
        for m in re.finditer(r"([A-Z]{2}\.[A-Z]{2})-(\d{1,2})\b", refs_text):
            cat = m.group(1)
            num_str = m.group(2)
            # 1-digit (1-9) with no leading zero → v1.1 style
            # 2-digit (10-12) → v1.1 style for PR.IP only
            if cat not in _V11_CATS:
                continue
            if len(num_str) == 1 or (cat == "PR.IP" and int(num_str) >= 10):
                v11_ids.append(f"{cat}-{num_str}")
        # Deduplicate while preserving order
        seen: set[str] = set()
        v11_ids = [v for v in v11_ids if not (v in seen or seen.add(v))]
        if v11_ids:
            out[cat_id] = [(row_idx, cat_text, v11_ids)]
    return out
