#!/usr/bin/env python3
"""Single M3 call for P1C-LLM-03-STRATEGIC-SYNTHESIS (global_reduce, case1-tinytask).

CORR-074 propagation dry-run for P1C-LLM-03. Validates that MiniMax-M3:
  1. Renders the v1.1.0 markdown template (## Status / ## Implications / ## Notes).
  2. Emits 1-N ### IMP-NN subsections with body fields.
  3. Parses successfully via P1CLLM03Parser with Pydantic validation passing.

Output artefacts (under /tmp/corr074-propagation/):
  - p1c03_system.txt / p1c03_user.txt (rendered prompts)
  - p1c03_raw.txt (M3 raw response)
  - p1c03_parse_result.json (parsed model summary)

NO env vars on CLI — loaded via `set -a && source .env && set +a` from the shell.
"""
    ...
    return {
        ...
        "doc07b_profile": [
            ...
            {"sub_domain_id": "D-04.3", "tier": "STANDARD",
             ...
             "verification_method": ["DEMONSTRATE"],
             "ownership": "COMPANY",
             "example_controls": ["PagerDuty incident workflow"]},
        ],
        "p1c_llm_01_outputs_by_domain": [
            {"domain_id": "D-01", "sub_domain_activations": [
                {"sub_domain_id": "D-01.1", "applicable": True},
            ]},
        ],
        "business_goals": [
            {"id": "BG-01", "description": "Achieve EU customer trust",
             "priority": "HIGH"},
        ],
        "company_facts": {
            "scale": "MICRO",
...
def main() -> int:
    ...
    (OUT_DIR / "p1c03_system.txt").write_text(system, encoding="utf-8")
    (OUT_DIR / "p1c03_user.txt").write_text(user, encoding="utf-8")
    ...
    (OUT_DIR / "p1c03_raw.txt").write_text(text, encoding="utf-8")
    ...
    if model is None:
        ...
        for imp in model.implications:
            print(f"    - {imp.id} subs={imp.affected_sub_domains} regs={imp.regulations} "
                  f"risk={imp.risk_level.value} conf={imp.confidence.value}")
        print(f"  notes_len={len(model.notes)}")

    (OUT_DIR / "p1c03_parse_result.json").write_text(
