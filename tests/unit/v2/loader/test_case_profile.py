...
def ctx() -> CompanyContext:
    """Module-scoped fixture: shared CompanyContext for case1-tinytask."""
...
def test_company_name_canonical(ctx: CompanyContext) -> None:
...
def test_company_size_micro(ctx: CompanyContext) -> None:
...
def test_company_jurisdiction(ctx: CompanyContext) -> None:
    assert "Portugal" in ctx.company.jurisdiction
    assert "EU" in ctx.company.jurisdiction


def test_company_security_fte(ctx: CompanyContext) -> None:
...
def test_applicability_predicates_gdpr_cra(ctx: CompanyContext) -> None:
...
def test_applicability_predicates_nis2_dora_aiact(ctx: CompanyContext) -> None:
...
def test_applicable_regs_computed(ctx: CompanyContext) -> None:
...
def test_declared_applicable_regs(ctx: CompanyContext) -> None:
    """Declared from regulatory/applicability.yaml (sorted)."""
    assert set(ctx.declared_applicable_regs) == {"GDPR", "CRA"}


def test_no_declaration_gaps(ctx: CompanyContext) -> None:
...
def test_obligated_party_per_reg(ctx: CompanyContext) -> None:
...
def test_clause_count_per_reg(ctx: CompanyContext) -> None:
...
def test_regulatory_non_applicable(ctx: CompanyContext) -> None:
...
def test_business_goals_count(ctx: CompanyContext) -> None:
...
def test_business_goals_have_priority(ctx: CompanyContext) -> None:
    bg1 = next(g for g in ctx.business_goals if g.id == "BG-01")
    assert bg1.priority == "HIGH"


def test_stakeholders_count(ctx: CompanyContext) -> None:
...
def test_stakeholder_responsibilities(ctx: CompanyContext) -> None:
...
def test_architecture_systems(ctx: CompanyContext) -> None:
    """SYS-01..SYS-05 per systems.yaml."""
    assert len(ctx.architecture.systems) == 5


def test_architecture_auth_systems(ctx: CompanyContext) -> None:
    """AS-01..AS-03 per auth_systems.yaml."""
    assert len(ctx.architecture.auth_systems) == 3


def test_architecture_other_sections_present(ctx: CompanyContext) -> None:
...
def test_load_is_idempotent(ctx: CompanyContext) -> None:
...
def test_models_are_pydantic(ctx: CompanyContext) -> None:
