"""Unit test for Case 3 (OmniBank Financial Systems S.A.) deterministic pipeline."""
...
def test_case3_deterministic_pipeline(tmp_path: Path) -> None:
    case_dir = Path("cases/case3-omnibank")
    assert case_dir.exists(), "Case 3 directory missing"

    # 1. Test profile loader
    profile = CaseProfileLoader(case_dir).load()
    assert profile.company.name == "OmniBank Financial Systems S.A."
    assert profile.company.scale == "LARGE"
    assert set(profile.applicable_regs) == {"AI_Act", "CRA", "DORA", "GDPR", "NIS2"}
    ...
    assert "OmniBank Financial Systems S.A." in doc04_text
    assert "HIGH" in doc04_text
    ...
    assert fm["case_study"] == "OmniBank Financial Systems S.A."
    assert set(fm["applicable_regs"]) == {"AI_Act", "CRA", "DORA", "GDPR", "NIS2"}
    assert "✅ AI_Act" in doc07_text
    assert "✅ CRA" in doc07_text
    assert "✅ DORA" in doc07_text
    assert "✅ GDPR" in doc07_text
    assert "✅ NIS2" in doc07_text

    # 5. Verify Excel workbook
    xlsx_path = Path(written["AEGIS-P1-XLSX"])
    assert xlsx_path.name == "Case_03_Phase1.xlsx"
