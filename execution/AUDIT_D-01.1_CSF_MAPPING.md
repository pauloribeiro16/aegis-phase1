...
**Date:** 2026-07-19 (original) / 2026-07-20 (closed by CORR-027)
**Auditor:** AEGIS-KG (manual, pre-automation)
**Closed by:** AEGIS-P1-CORR-027 (outcomes below)
...
### 2.1 Stale mapping: `PR.DS-12` is dropped from CSF 2.0

| Old (CSF 1.1) | New (CSF 2.0) | Status |
|---|---|---|
| `PR.DS-12` | (no mapping) | **DROPPED by CORR-027** |

**Outcome (CORR-027 Phase 2):** The original analysis conflated two separate things:

- `PR.DS-1` (v1.1, "Data-at-rest is protected") → `PR.DS-01` (v2.0, "data-at-rest") — this is an identity rename, **NOT** a withdrawal.
- `PR.DS-12` (the v2.0 ID with the title "Data is managed consistent with the organization's risk strategy…") is **not present in NIST CSWP 29**. The original audit's claim that it was "withdrawn" was incorrect — it's a pre-finalization draft artifact that never made it into the published CSF 2.0. The active PR.DS subcategory set in CSF 2.0 is **`PR.DS-01`, `PR.DS-02`, `PR.DS-10`, `PR.DS-11`** — four subcategories, not five.

CORR-027 Phase 2 (aegis-phase1 commit `8d9f1c5` + Methodology-main commit on `feature/aia-cluster-d-postmarket`) removed the `PR.DS-12` row from the .md, added a `## Decisions (CORR-027)` section (D1) documenting the rationale, and added a CI gate (`.hooks/ci-csf-frozen-list.sh`) that fails the build if the .md ever drifts from the xlsx-derived truth. **This finding is closed.**
...
1. **Fix `participating_regulations`** in D-01.1: `[]` → `["GDPR", "NIS2", "CRA", "DORA"]` — **DONE** (CORR-024 v11a)
2. **Expand `csf_hint`** from 2 to 8 IDs (add `PR.DS-10, PR.AA-01, PR.AA-05, ID.AM-08, GV.RM-04, DE.CM-09`) — **DONE** (CORR-024 v11a, plus `PR.DS-10` and `PR.DS-11` retained)
3. **Fill `nist_csf_mapping` per-SR** (see table above) — **DONE** (CORR-024 v11a, 4/4 SRs have CSF mappings)
...
4. Build a `tools/audit_csf_mapping.py` script that, for each D-XX:
   ...
   - **DONE** (CORR-027 Phase 3 — `scripts/preprocess/audit_csf_mapping.py`)

### Priority 3 (full automation)

5. LLM-assisted mapping expansion: feed D-XX source + CSF 2.0 glossary, get suggested mapping, human review, apply. — **DEFERRED** to CORR-029 (uses the audit report from Priority 2 as ground truth).

## 4. Decisions to Take

Before I proceed:

- [x] **Expand `csf_hint` to 8 IDs** (priority 1.2 above) for D-01.1 manually? — **DONE** (CORR-024 v11a; now 8 IDs)
- [x] **Fill `nist_csf_mapping` per-SR** (priority 1.3) for D-01.1 manually? — **DONE** (CORR-024 v11a; 4/4 SRs have mappings)
- [x] **Fix `participating_regulations`** (priority 1.1)? — **DONE** (CORR-024 v11a; now `["GDPR", "NIS2", "CRA", "DORA"]`)
- [x] **Build the audit tool first** (priority 2) — generate the report for all 38 subdomains so we can see the full picture? — **DONE** (CORR-027 Phase 3; `scripts/preprocess/audit_csf_mapping.py` + `preproc_out/audit/csf_mapping_report.json`)
- [ ] **Defer to automation** — just build the LLM-assisted mapping tool? — **DEFERRED** to CORR-029 (uses the audit report as ground truth)
