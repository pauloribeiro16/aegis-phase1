...
- **`qwen3.8:27b` cannot be evaluated on this cluster without an
  ...
  **UPDATE 2026-09-01** — see §2.9 below: `qwen3.8:27b` IS now
  runnable on Deucalion. The pre-installed Ollama 0.31.1 cannot run
  it (HTTP 500 `unknown renderer "qwen3.8"`), but the Ollama **0.32.13**
  binary that already lives in
  `/projects/F202512235CPCAA1/graphify-methodology/{bin,lib}` works
  (validated in graphify JOB 1825982, 2026-08-15, then again in the
  aegis scout JOB 1862819 below).

---

## 2.9 JOB 1862819 — `qwen3.8:27b` aegis scout (SUCCEEDED, 2026-09-01)

**Goal**: verify `qwen3.8:27b` can run the aegis Phase 1B end-to-end on
Deucalion. The model blob was already cached but unusable until the
right Ollama binary was wired.

| Field | Value |
|-------|-------|
| Submitted | 2026-09-01 14:33 WEST (`paulinho` via SSH to `login.deucalion.macc.fccn.pt`) |
| JOB ID | **1862819** |
| Partition | `dev-a100-80` |
| Walltime | 30 min |
| **Elapsed** | **18:04** |
| Exit | **0:0 (SUCCESS)** |
| Node | gnx508.deucalion.macc.fccn.pt |
| GPU | 1×A100-80GB; qwen3.8:27b loaded 18 GB / 80 GB (100% GPU) |
| Ollama | **0.32.13** from `/projects/F202512235CPCAA1/graphify-methodology/{bin,lib/ollama}` |
| Script | `aegis-phase1/examples/deucalion/scout-qwen38-aegis.sbatch` |

### Per-call profile (Phase 1B, case1-tinytask, 2 regulations)

| # | Spec | Reg | Elapsed | Tokens | tok/s |
|---|------|-----|--------:|-------:|------:|
| 1 | P1B-LLM-01-INTERPRETATION | CRA  | 201s | 122 599 | ~610 |
| 2 | P1B-LLM-02-RATIONALE      | CRA  | 218s | 126 261 | ~579 |
| 3 | P1B-LLM-01-INTERPRETATION | GDPR | 152s |  92 775 | ~610 |
| 4 | P1B-LLM-02-RATIONALE      | GDPR | 188s |  96 014 | ~511 |

- **LOAD**: 38 sub-domains, 2 regs, 2.46 s
- **Phase 1B complete**: `rationale_by_reg has 2 entries` (CRA + GDPR)
- **Output artefact**: `output/scout_qwen38_1862819/05_Regulatory_Applicability.md` (42 KB)

### Why this matters

- **`qwen3.8:27b` is the first non-M3 model in the corr044 family matrix
  to produce a CORR-074-compliant structured output natively** — the
  Doc 05 markdown includes `## Status` / `## Interpretations` /
  `## Derogations` / `## Findings` / `## Rationale` sections with
  `applicable`, `confidence`, `IMP-*` and `GAP-*` records carrying
  `Effort`, `Dependencies`, `layer0_refs`, `company_fact_refs`. The
  P1B-01 parser and the P1B-02 parser both accept it cleanly without
  bullet-tolerant fallback.
- **Throughput is ~52% higher than qwen3.5** (≈ 600 tok/s vs ≈ 395
  tok/s on the same hardware — A100-80), and **all 4 calls returned
  within walltime** (the qwen3.5 scout used 17:30 of a 30 min budget;
  qwen3.8 used 18:04).
- **Reasoning quality is a step change over qwen3.5**:
  - `IMP-D-04.3-1` cites CRA Art. 14(1) 24 h AEV SLA, the
    `DOC04:SEC-02 incident_response=PARTIAL` readiness, and the
    `DOC04:SEC-01 role_matrix.cra.role=manufacturer` anchor.
  - `GAP-D-02.1` carries `priority=P1`, `coverage_level=
    NOT_ADDRESSED`, and `recommendation: Generate initial SBOM from
    GitHub Actions dependency manifests; document and accept residual
    risk for transitive dependencies not yet inventoried`.
  - `GAP-D-09.4` surfaces the `Annex VII` documentation pack gap and
    links it to the `CLASS_I` Module A assessment requirement, with
    priority P1.
  - Rationale prose in `§6.1b` (CRA) is 7 356 chars; GDPR rationale is
    6 782 chars — comparable in volume to the qwen3.5 run-all outputs
    but with the structured CORR-074 envelope.
- **No HTTP 500 `unknown renderer "qwen3.8"`** — confirming that the
  Ollama 0.32.13 binary carries the renderer that 0.31.1 lacks.

### Caveats and follow-ups

- **Phase 1B only** — MAP (10× P1C-01) and REDUCE (P1C-02/03) were
  not exercised in this scout. The qwen3.5 run-all (JOB 1847659) is
  the reference for the full-pipeline picture; qwen3.8 P1C behaviour
  has not yet been measured.
- **The Method that worked** is captured in `scout-qwen38-aegis.sbatch`
  and reusable for any future aegis-job that needs `qwen3.8:27b`.
- **Next step options** (for a follow-up contract):
  - A) run `--run-all` (8 h) on qwen3.8 to measure P1C-01 template
    compliance — the open question after the qwen3.5 run-all.
    **SUBMITTED 2026-09-01 as JOB 1862843** (`run-qwen38-full.sbatch`,
    8 h, normal-a100-80); in flight at this revision.
  - B) run a same-stage comparison scout (qwen3.8 vs qwen3.5 vs M3
    gold) on a different case (case2-secureborder) to test cross-case
    generalisation.

### Scorecard (cross-link to `model_matrix.md`)

The per-model scorecard lives in `execution/reports/model_matrix.md`
(judge = GLM-5.3-Flash via ZCode, contract CORR-105). Per-run digests
are in `execution/reports/digests/`. As of this scout update:
