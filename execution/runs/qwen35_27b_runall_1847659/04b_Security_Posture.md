...
version: 1.0
status: DRAFT
generated_at: "2026-08-24T14:47:20Z"
phase: 1
created: "2026-08-24T14:47:20Z"
updated: "2026-08-24T14:47:20Z"
author: Executor
...
**Target maturity**: 2  
**Gap**: 1  
**Notes**: Current maturity for domain D-01 is level 1, requiring alignment of Engineering workflows with GDPR Art. 32 and CRA Annex I mandates on Encryption at rest, in transit, Key management, and Data integrity. Closing the gap to target maturity involves CISO oversight of security architecture while Engineering implements verified cryptographic solutions to replace ad-hoc configurations. Formalizing these processes ensures sustained compliance for data protection controls without assigning responsibility to individuals or specific titles.
...
**Target maturity**: 2  
**Gap**: 1  
**Notes**: Current maturity level one indicates insufficient implementation of patch management cadence and vulnerability scanning which fails to satisfy GDPR Article 32 security standards or CRA Annex I resilience controls, requiring Engineering to establish repeatable technical processes. To close the gap to target maturity two, CISO oversight is required to validate penetration testing cycles while Engineering implements SBOM and dependency tracking alongside enforced remediation timelines for all identified risks. This coordinated effort ensures alignment with applicable regulations through systematic vulnerability identification and response mechanisms.
...
**Target maturity**: 2  
**Gap**: 1  
**Notes**: To bridge the maturity gap in Domain D-03, Engineering will implement technical controls for identity authentication and session management aligned with GDPR Article 32 requirements. Furthermore, Governance must formalize access policies under CISO oversight to enforce Privileged Access Management protocols and segregate duties consistent with CRA Annex I standards.
...
**Target maturity**: 2  
**Gap**: 1  
**Notes**: Current maturity level 1 reflects ad-hoc incident response processes where Engineering lacks integration with Governance regarding CRA Annex I notification protocols requiring CISO oversight. Achieving target maturity requires Engineering to establish detection monitoring aligned with GDPR Art. 32 while Operations formalizes forensic evidence capture procedures under established playbooks. Critical remediation involves defining post-incident review workflows between the DPO and CISO functions to ensure complete regulator communication compliance within domain D-04 controls.
...
**Target maturity**: 1  
**Gap**: 0  
**Notes**: The Data Lifecycle domain meets target maturity with zero gaps as **Operations** executes data retention schedules under **CISO** oversight while **Engineering** manages backup recovery and lineage tracking per GDPR Article 32 requirements. Since current capabilities satisfy CRA Annex I controls for data classification, no material remediation items are required to address any compliance deficits within the lifecycle scope. Ongoing review by the **DPO** confirms these functions sustain regulatory alignment across all stages without introducing new operational overhead.
...
**Target maturity**: 1  
**Gap**: 0  
**Notes**: Current supply chain processes satisfy GDPR Article 32 and CRA Annex I requirements with a zero-gap status, maintaining target maturity through existing vendor due diligence and exit strategies without requiring immediate remediation items. Governance oversees contracts and data processing agreements while Operations manages sub-processor oversight to control concentration risk within the approved regulatory framework. The DPO validates these arrangements to ensure continuous compliance, confirming that established controls meet all material obligations under AEGIS domain D-06 standards.
...
**Target maturity**: 2  
**Gap**: 1  
**Notes**: Engineering currently utilizes ad-hoc development practices that fail to meet the security of processing mandates under GDPR Article 32 and CRA Annex I controls for digital element integrity. Remediation requires integrating automated SAST, dependency scanning, and secrets management into SDLC workflows with CISO oversight to enforce secure coding standards during change and release management. Implementing these capabilities will elevate maturity by formalizing risk mitigation procedures necessary for regulatory compliance prior to software deployment.
...
**Target maturity**: 1  
**Gap**: 0  
**Notes**: Governance collaborates with DPO functions to enforce GDPR Article 32 requirements through mandatory background checks and role-based training for all Engineering personnel. Operations executes security awareness programs and phishing simulations under CISO direction to satisfy CRA Annex I human factor controls while maintaining the established target maturity. Establishment of offboarding protocols and personnel vetting serves as the primary remedial baseline ensuring regulatory adherence within this gap-free maturity state.
...
**Target maturity**: 2  
**Gap**: 1  
**Notes**: The current maturity level one state lacks a formalized Policy framework and Risk management process required to satisfy GDPR Article 32 security measures and Cyber Resilience Act Annex I documentation controls. Remediation requires the Governance function to define organizational oversight structures while the DPO ensures alignment of Board accountability through a new Compliance program with structured Internal audit procedures to achieve target maturity level two.
...
**Target maturity**: 1  
**Gap**: 0  
**Notes**: Governance and Engineering maintain alignment with GDPR Article 32 and CRA Annex I by sustaining Log collection (SIEM) and Metrics KPIs at maturity level one without identified gaps for remediation. CISO oversees Threat intelligence while Operations manages Anomaly detection to ensure Detection use cases remain compliant, focusing on these material control implementations rather than corrective actions due to the zero gap status. DPO oversight ensures continuous verification of this baseline state across all applicable regulatory controls within domain D-10.
...
| --- | --- | --- | --- |
| 1.0 | 2026-08-24 | Executor | Generated AEGIS maturity assessment from state[architecture_inventory] and ontology |

## N. Document Approval

| Role | Name | Signature | Date |
| --- | --- | --- | --- |
| Document Author | Executor |  | 2026-08-24 |
