...
phase: 1
version: 1.0
created: "2026-09-01T14:33:25Z"
updated: "2026-09-01T14:33:25Z"
author: Executor
status: DRAFT
...
generated_at: "2026-09-01T14:33:25Z"
---
# AEGIS-P1-04a Architecture & Data Inventory

## 1. Technical Architecture

TinyTask Lda. operates a data-processing architecture spanning 37 active sub-domains (with D-08.3 excluded as inactive) under the combined obligations of the Cyber Resilience Act and GDPR, in which the Engineering function is the accountable party for all cryptographic controls—encryption at rest (CAP-D01-001) and encryption in transit (CAP-D01-002)—under the supervisory responsibility of the CISO function. Data-flow integrity across the in-scope inventory of stores and processing pipelines is maintained through the Operations function's execution of retention-and-deletion policies (CAP-D01-004) and forensic evidence-capture procedures (CAP-D04-005), with the CISO function retaining supervisory authority over both capabilities and the Governance function serving as accountable party for the latter. Regulatory notification and supervisor-liaison obligations (CAP-D04-003) are assigned to the Governance function under CISO supervision, ensuring a single coordinated response channel to the relevant data-protection authorities. The DPO function, maintained at 0.2 FTE, provides the GDPR data-protection governance layer across all active sub-domains, while the Engineering function constitutes the sole technical implementation tier responsible for the design, build, and operational hardening of every in-scope system.
...
End users reach SYS-01 directly over a single TLS-encrypted public endpoint with no intermediate WAF, CDN, or DMZ zone, so the Engineering function's application layer constitutes the sole ingress control. Internal traffic between SYS-01 and SYS-03 traverses a flat, unsegmented virtual network under the Operations function's management, with data persisted in SYS-03 encrypted at rest. Administrative and operational access to both systems is performed by the Engineering and Operations functions through direct cloud-console or SSH sessions, with no bastion host, PAM gateway, or jump server interposed in the path. The topology contains no SOC, no SIEM or centralized log collector, and no dedicated network zones beyond the single flat VPC, leaving the CISO and Governance functions without a continuous monitoring or alerting feed to draw upon.
