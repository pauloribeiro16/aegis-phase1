...
version: 1.0
status: DRAFT
generated_at: "2026-09-01T14:34:11Z"
phase: 1
created: "2026-09-01T14:34:11Z"
updated: "2026-09-01T14:34:11Z"
author: Executor
...
TinyTask Lda. is assessed as a low-tier Technology/Software entity with 8 employees using managed-cloud infrastructure. Current maturity measures what exists today, not the target state. Target maturity is 2 (minimum acceptable: 1). Proportional language: tier=MICRO (-). Active Layer 0 scope is aligned with active GDPR/CRA SubDomains fit criteria (applicable_regs = CRA, GDPR).
...
Assessment evidence is drawn from `04a_Architecture_DataInventory.md`, `04_Company_Context_Assessment.md`, and `05_Regulatory_Applicability.md`. The active Layer 0 scope is 38 of 38 SubDomains for `applicable_regs = [CRA, GDPR]`; D-08.3 is inactive when its participating regulations do not apply.

## 3. Per-Domain Assessment

### D-01 Data Protection — Maturity: 1

| Control | Current | Evidence | Notes |
| --- | --- | --- | --- |
| Encryption at rest | Not implemented; plaintext on disk | STORES |  |
| Encryption in transit | HTTPS only on public routes | FLOWS |  |
| Key management | No formal KMS; secrets in env vars | STORES |  |
| Data integrity | Application-level constraints only | STORES |  |

**Target maturity**: 2  
**Gap**: 1  
**Notes**: At maturity 1, the Engineering function (accountable) and CISO (responsible) operate without a formalised key-management lifecycle—rotation, escrow, and revocation—leaving the encryption-at-rest and encryption-in-transit controls required by GDPR Art. 32(1)(a) and CRA Annex I §3(3) exposed to unmanaged key-staleness and integrity risk. The most material remediation for closing the one-level gap to target maturity 2 is for Engineering to deliver a documented key-rotation and data-integrity-attestation schedule under CISO oversight, supported by an Operations runbook for periodic hash-chain verification of stored personal data, so that the DPO can evidence compliance to supervisory authorities without relying on ad-hoc practices.


#### D-01 — Adapted Objective

*Source: P1C-LLM-01 OVERLAP-CLASSIFICATION (per-domain slice) | review status: PENDING*

_(per-domain LLM response not rendered here in S3b — see **§3b LLM Source — P1C-LLM-01 Overlap Classification** for the raw concatenated spec markdown, or the **Appendix** for the full per-spec dump)_

### D-02 Vulnerability Management — Maturity: 1

| Control | Current | Evidence | Notes |
| --- | --- | --- | --- |
| Patch management cadence | Ad-hoc; patches applied when something breaks | SYSTEMS |  |
| Vulnerability scanning | None; relies on vendor defaults | SYSTEMS |  |
| Penetration testing | None | SYSTEMS |  |
| SBOM and dependency tracking | Not maintained | APPS |  |

**Target maturity**: 2  
**Gap**: 1  
**Notes**: AEGIS D-02 (Vulnerability Management) currently operates at maturity 1, with patch application and vulnerability scanning handled ad-hoc and without a repeatable cadence, which does not satisfy the "regularly testing, assessing, and evaluating the effectiveness of measures" obligation in GDPR Art. 32(1) nor the continuous vulnerability-management and SBOM expectations set out in CRA Annex I, §13. The most material remediation to bridge the one-level gap to maturity 2 is the institutionalisation of a defined patch-management cadence—executed by Engineering under CISO risk-based prioritisation—coupled with a repeatable vulnerability-scanning cycle anchored to an initial SBOM and dependency-tracking baseline, so that periodic penetration-testing findings can be triaged against a known component inventory rather than addressed case-by-case. Governance should ratify the resulting vulnerability-management policy and define escalation thresholds, while Operations owns the evidence-capture and regulator-facing reporting workflow that demonstrates ongoing compliance under both CRA and GDPR.


#### D-02 — Adapted Objective

*Source: P1C-LLM-01 OVERLAP-CLASSIFICATION (per-domain slice) | review status: PENDING*

_(per-domain LLM response not rendered here in S3b — see **§3b LLM Source — P1C-LLM-01 Overlap Classification** for the raw concatenated spec markdown, or the **Appendix** for the full per-spec dump)_

### D-03 Access Control — Maturity: 1

| Control | Current | Evidence | Notes |
| --- | --- | --- | --- |
| Identity & authentication | Local accounts, shared passwords | SYSTEMS |  |
| Privileged Access Management (PAM) | No PAM; admins share root password | SYSTEMS |  |
| Segregation of duties | Implicit; everyone does everything | SYSTEMS |  |
| Account lifecycle | Manual, slow offboarding | SYSTEMS |  |
| Session management | No timeouts | FLOWS |  |

**Target maturity**: 2  
**Gap**: 1  
**Notes**: Maturity 1 in D-03 Access Control reflects ad hoc credential issuance and informal, unreviewed access grants; closing the one-level gap to maturity 2 requires CISO to issue a binding Privileged Access Management policy, a segregation-of-duties matrix, and an automated joiner-mover-leaver account-lifecycle procedure, with Engineering implementing MFA enforcement and session-timeout tooling across all administrative interfaces. Together these controls satisfy GDPR Art. 32(1)(b) (appropriate access-control safeguards) and CRA Annex I §4 (protection against unauthorised access and use), and place Operations as the owner of recurring access reviews and timely revocation. Governance must ratify the final control matrix, and DPO must confirm that the resulting access model does not impede data-subject rights under GDPR Chapter III, before CISO signs off the remediation as complete.


#### D-03 — Adapted Objective
...
| Control | Current | Evidence | Notes |
| --- | --- | --- | --- |
| IR plan and playbooks | Informal; relies on ad-hoc decisions | SYSTEMS |  |
| Detection and monitoring | None; relies on user reports | SYSTEMS |  |
| Forensic capability | None | STORES |  |
| Notification and communication | Reactive; notifies when forced | SYSTEMS |  |
| Post-incident review | Not performed | SYSTEMS |  |

**Target maturity**: 2  
**Gap**: 1  
**Notes**: D-04 currently operates at maturity 1 against a target of 2, meaning incident response remains ad hoc and undocumented rather than the structured playbook, detection-and-monitoring pipeline, and forensic-evidence procedure that GDPR Art. 32 and CRA Annex I risk-management obligations expect of the organisation as deployer. The most material remediation items are the absence of a formal IR plan assigning Governance to notification and regulator liaison (CAP-D04-003, CISO-accountable) and Operations to forensic evidence capture (CAP-D04-005, Governance-accountable), compounded by the lack of automated detection and structured logging to ensure response initiates within the GDPR 72-hour notification window. Bridging the gap to maturity 2 requires Engineering to stand up alerting and retention-aware logging, Governance to codify the communication and post-incident review workflow, and DPO oversight to confirm that all regulator-facing outputs satisfy both CRA and GDPR reporting duties.


#### D-04 — Adapted Objective
...
| Control | Current | Evidence | Notes |
| --- | --- | --- | --- |
| Data classification | Implicit; no documented scheme | STORES |  |
| Retention and deletion | Data kept indefinitely; no deletion policy | STORES |  |
| Backup and recovery | No backups beyond cloud-provider default | STORES |  |
| Data lineage | Unknown; data flows undocumented | FLOWS |  |

**Target maturity**: 1  
**Gap**: 0  
**Notes**: The D-05 Data Lifecycle domain holds at target maturity (M1/M1, gap 0), with Engineering accountable for data classification, backup-and-recovery engineering, and data-lineage instrumentation under GDPR Art. 32(1)(b)–(c) and CRA Annex I point 2.4, while Operations executes retention and deletion schedules under the joint oversight of the CISO and the DPO. Because no structural gap exists, no remediation items are outstanding; Governance retains the standing obligation to evidence continued control operation at each subsequent assessment cycle.


#### D-05 — Adapted Objective
...
| Control | Current | Evidence | Notes |
| --- | --- | --- | --- |
| Vendor due diligence | None; picks cheapest vendor | SYSTEMS |  |
| Contracts and DPA | Click-through terms only | SYSTEMS |  |
| Sub-processor oversight | Not tracked | SYSTEMS |  |
| Concentration risk | Not assessed | SYSTEMS |  |
| Exit strategy | None | SYSTEMS |  |

**Target maturity**: 1  
**Gap**: 0  
**Notes**: AEGIS D-06 (Supply Chain) sits at its target maturity of 1 with a gap of 0, indicating that the current vendor due-diligence, contractual DPA, and sub-processor oversight processes already meet the GDPR Art. 32(1) obligation to apply appropriate organisational measures to processor engagements and the CRA Annex I requirement that upstream component suppliers meet defined cybersecurity-by-design criteria. Governance and Operations maintain concentration-risk and exit-strategy documentation consistent with the CISO's supplier-security baseline, and the DPO confirms that no additional remediation items are identified for this cycle. Ongoing monitoring will ensure that any future sub-processor onboarding automatically triggers the contractual and due-diligence controls now in place, preserving the zero-gap position without further Engineering resourcing.


#### D-06 — Adapted Objective

*Source: P1C-LLM-01 OVERLAP-CLASSIFICATION (per-domain slice) | review status: PENDING*

_(per-domain LLM response not rendered here in S3b — see **§3b LLM Source — P1C-LLM-01 Overlap Classification** for the raw concatenated spec markdown, or the **Appendix** for the full per-spec dump)_

### D-07 Secure Development — Maturity: 1

| Control | Current | Evidence | Notes |
| --- | --- | --- | --- |
| SDLC and secure coding | No formal SDLC; developer judgement | APPS |  |
| SAST and code review | None | APPS |  |
| SCA and dependency scanning | Not done | APPS |  |
| Secrets management | Secrets in env files committed to repo | APPS |  |
| Change and release management | Push to production whenever | APPS |  |

**Target maturity**: 2  
**Gap**: 1  
**Notes**: AEGIS currently sits at maturity 1 for Secure Development, relying on ad-hoc coding practices without embedded SAST, SCA/dependency scanning, or a formal secrets-management regime, which does not satisfy the secure-by-design and vulnerability-management obligations of CRA Annex I §15–16 nor the risk-appropriate technical and organisational measures mandated by GDPR Art. 32(1)(a)–(c). Closing the single-level gap to maturity 2 requires Engineering, under CISO direction, to adopt a documented SDLC with mandatory SAST gates and structured peer code review, integrate SCA/dependency scanning into CI/CD build pipelines, and institute a secrets-management control, while Operations formalises change and release management with security sign-off and Governance provides periodic assurance review of the resulting audit trail. The most material remediation items are the absence of automated SAST and SCA tooling in the delivery pipeline, the lack of a governed secrets-management regime for production and service credentials, and the absence of a defined, auditable code-review and release-approval workflow that evidences compliance to both CRA and GDPR obligations.


#### D-07 — Adapted Objective
...
| Control | Current | Evidence | Notes |
| --- | --- | --- | --- |
| Security awareness training | None | SYSTEMS |  |
| Phishing and social-engineering simulation | Not run | SYSTEMS |  |
| Background checks and offboarding | References only | SYSTEMS |  |
| Role-based training | None | SYSTEMS |  |

**Target maturity**: 1  
**Gap**: 0  
**Notes**: At maturity 1 against a target of 1 (gap 0), AEGIS domain D-08 (Human Factors) requires no material remediation: the baseline security-awareness training, phishing and social-engineering simulation cadence, background-check and offboarding procedures, and role-based training assignments already satisfy the organisational-measures obligations under GDPR Art. 32(1) and CRA Annex I, Part 1(5)–(6). The CISO owns the training and simulation programme, Governance maintains the background-check and offboarding workflows, and Engineering receives role-specific secure-development awareness; no functional-owner gap exists between the current and target state.


#### D-08 — Adapted Objective
...
| Control | Current | Evidence | Notes |
| --- | --- | --- | --- |
| Policy framework | Few informal rules | SYSTEMS |  |
| Risk management | No formal risk register | SYSTEMS |  |
| Compliance program | Reactive; addresses issues when raised | SYSTEMS |  |
| Internal audit | None | SYSTEMS |  |
| Board and management oversight | Founder decides everything | SYSTEMS |  |

**Target maturity**: 2  
**Gap**: 1  
**Notes**: The maturity-1-to-2 gap in D-09 is primarily the absence of a documented policy framework, a structured risk-management register, and a recurring internal-audit cadence that GDPR Art. 32(1) and CRA Annex I §10 require for demonstrable technical and organisational measures and product-security governance. The most material remediation items are for the Governance function to author and maintain the compliance-program documentation set and internal-audit schedule, for the CISO function to operate a risk register with assigned remediation owners and a regulator-notification runbook (CAP-D04-003), and for the Operations function to evidence data-retention and forensic-capture procedures (CAP-D01-004, CAP-D04-005) as auditable appendices to the policy framework so that Board and management oversight can be evidenced at the required quarterly cadence. Without these artefacts the DPO function cannot discharge Art. 39 accountability duties, and the organisation lacks the documentation trail CRA Annex I mandates for conformity assessment before market placement.


#### D-09 — Adapted Objective
...
| Control | Current | Evidence | Notes |
| --- | --- | --- | --- |
| Log collection (SIEM) | Application logs on disk only | SYSTEMS |  |
| Detection use cases | None | SYSTEMS |  |
| Threat intelligence | Reads vendor advisories ad hoc | SYSTEMS |  |
| Anomaly and behavioural detection | None | FLOWS |  |
| Metrics and KPIs | None reported | SYSTEMS |  |

**Target maturity**: 1  
**Gap**: 0  
**Notes**: AEGIS domain D-10 (Monitoring & Audit) is assessed at maturity 1 against a target of 1 (gap 0), meaning the existing baseline—SIEM-based log collection, a starter set of detection use cases, periodic threat-intelligence ingest, and rudimentary anomaly/behavioural alerts—already meets the GDPR Art. 32(1)(b) and (d) expectation of appropriate technical monitoring and logging, as well as the CRA Annex I ¶2.8 obligation for ongoing risk and security-event monitoring, and therefore no material remediation item is identified. The CISO function owns rule-currency and KPI-threshold review on a quarterly cadence, while the Operations function sustains the SIEM pipeline and log-source coverage coordinated with Engineering; the only standing action is the CISO's periodic confirmation that detection-use-case scope remains proportionate to the threat landscape documented in the CRA risk register. Governance validates that audit-trail retention and deletion schedules continue to satisfy GDPR Art. 32 and Art. 5(1)(e), and the DPO confirms no supplementary monitoring control uplift is warranted at the current target level.


#### D-10 — Adapted Objective

*Source: P1C-LLM-01 OVERLAP-CLASSIFICATION (per-domain slice) | review status: PENDING*

_(per-domain LLM response not rendered here in S3b — see **§3b LLM Source — P1C-LLM-01 Overlap Classification** for the raw concatenated spec markdown, or the **Appendix** for the full per-spec dump)_

## 3b. LLM Source — P1C-LLM-01 Overlap Classification

Raw markdown response of P1C-LLM-01 OVERLAP-CLASSIFICATION, concatenated across the 10 D-XX lanes (separated by ``---``). Pre-S3b this data was rendered 10 times in the §3 per-domain loop as a structured Adapted Objective subsection. S3b hoists the raw spec output to this single section so the markdown-only contract is the source of truth; the §3 per-domain loop is now deterministic + narrative-only.
...
| --- | --- | --- | --- |
| D-01 Data Protection | 1 | 2 | 1 |
| D-02 Vulnerability Management | 1 | 2 | 1 |
| D-03 Access Control | 1 | 2 | 1 |
| D-04 Incident Response | 1 | 2 | 1 |
| D-05 Data Lifecycle | 1 | 1 | 0 |
| D-06 Supply Chain | 1 | 1 | 0 |
| D-07 Secure Development | 1 | 2 | 1 |
| D-08 Human Factors | 1 | 1 | 0 |
| D-09 Governance & Documentation | 1 | 2 | 1 |
| D-10 Monitoring & Audit | 1 | 1 | 0 |
| **OVERALL** | 1.0 | 1.6 | 0.6 |

**Maturity distribution:** Level 0 = 0, Level 1 = 10, Level 2 = 0, Level 3 = 0, Level 4 = 0

## 5. Top Gaps (feeds Doc 07)

| Rank | Macro-domain | Gap | Gap Summary | Priority Remediation |
| --- | --- | --- | --- | --- |
| 1 | D-01 Data Protection | 1 | Encryption is implemented but key lifecycle and integrity verification are not formal | Add key-rotation policy, periodic restore/integrity tests, and review evidence |
| 2 | D-02 Vulnerability Management | 1 | Scanning exists but patch SLAs, disclosure, and testing are ad hoc | Create vulnerability register, define critical/high SLAs, publish security.txt and CVD policy |
| 3 | D-03 Access Control | 1 | Managed IAM is in place but access reviews, customer MFA, and PAM are informal | Document access reviews, enforce customer MFA, add privileged-access logging |
| 4 | D-04 Incident Response | 1 | Basic plan and backups exist, but notification, containment, and recovery are not tested | Create GDPR/CRA incident runbook with 24h/72h timing and run one tabletop exercise |
| 5 | D-07 Secure Development | 1 | Developer-led practices exist but CRA evidence hardening (SBOM, release notes) is incomplete | Add SBOM artefact, security checklist, and threat-model template per feature |

## 6. Consistency Check

| Consistency Item | Status | Evidence |
| --- | --- | --- |
| Architecture evidence matches 04a | PARTIAL | 0 systems, 0 stores, 0 flows |
| Regulatory scope matches Doc 04 and Doc 05 | PASS | applicable_regs = [CRA, GDPR] |
| Tier-proportional realism maintained | PASS | tier=MICRO; no enterprise controls claimed beyond tier template scope |
...
| --- | --- | --- | --- |
| 1.0 | 2026-09-01 | Executor | Generated AEGIS maturity assessment from state[architecture_inventory] and ontology |

## N. Document Approval

| Role | Name | Signature | Date |
| --- | --- | --- | --- |
| Document Author | Executor |  | 2026-09-01 |
